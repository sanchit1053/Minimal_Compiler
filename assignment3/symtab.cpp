...
int entity::getOffset(int paramOffset, int isFunction){
    if(this->entityType == Function){
        ...
        if(this->scope == Parameter){
            int change = 8 - paramOffset;
            return this->offset + change;
        }
        else{
            int o = 0;
            if(isFunction == -1){
            ...
            return isFunction * (this->offset + o ) ;
        }
    }
}
...
int symtab::getOffset(std::string s){
    return this->getEntity(s)->getOffset(this->parameterOffset, -1);
}

int symtab::getOffsetStruct(std::string s){
    return this->getEntity(s)->getOffset(0, 1);
}